...
interface BacklogRequestRepository : JpaRepository<BacklogRequest, UUID> {
  ...
  @Query(
    "SELECT b FROM BacklogRequest b " +
      ...
      " = " +
      "(SELECT COUNT(DISTINCT cfg.id) FROM ServiceConfiguration cfg WHERE cfg.enabled is TRUE)",
  )
  ...
  @Query(
    "SELECT b FROM BacklogRequest b " +
      "WHERE b.id = :id " +
      "AND EXISTS (" +
      "SELECT s FROM ServiceSummary s WHERE s.backlogRequest.id = :id AND s.dataHeld is true" +
      ")",
  )
