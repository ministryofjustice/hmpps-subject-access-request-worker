...
class PdfServiceTest {
  ...
  fun generatePdf() {
    val pdfStream = pdfService.execute(
      services = listOf(DpsService(name = "hmpps-book-secure-move-api1", orderPosition = 1, content = getTestData())),
      nomisId = "1234",
  ...
  fun generateReports() {
    ...
    pdfService.execute(
      ...
      println("Writing original SAR report ${stream.size()}")
      Files.write(Paths.get(PDF_V1), stream.toByteArray())
    }

    PdfService().generateSubjectAccessRequestPDF(params).use { stream ->
      println("Writing refactored SAR report ${stream.size()}")
      Files.write(Paths.get(PDF_V2), stream.toByteArray())
    }
  }
