...
class SubjectAccessRequestWorkerServiceTest : IntegrationTestBase() {
  ...
  fun `startPolling calls claim and complete on happy path`() = runTest {
    ...
    Mockito.`when`(mockGetSubjectAccessRequestDataService.generatePDF(any(), any(), any(), any(), any(), any()))
  ...
  fun `doReport calls data service generatePDF`() {
    ...
    Mockito.`when`(documentGateway.storeDocument(UUID.fromString("11111111-1111-1111-1111-111111111111"), pdfStreamMock))
      .thenReturn("")
    subjectAccessRequestWorkerService.doReport(sampleSAR)
    verify(mockGetSubjectAccessRequestDataService, Mockito.times(1)).generatePDF(any(), any(), any(), any(), any(), any())
  }
  ...
  fun `doReport calls storeSubjectAccessRequestDocument`() = runTest {
    ...
    Mockito.`when`(mockGetSubjectAccessRequestDataService.generatePDF(any(), any(), any(), any(), any(), any())).thenReturn(mockByteArrayOutputStream)
