...
class TemplateRenderServiceTest {
  ...
  @Test
  fun `getServiceTemplate returns a test service template`() {
    val testTemplate = templateRenderService.getServiceTemplate("test-service")
    assertThat(testTemplate).contains("Test Data")
    // todo add full template check?
  }

  @Test
  fun `getServiceTemplate returns a null if service template does not exist`() {
    val testTemplate = templateRenderService.getServiceTemplate("null-service")
    assertThat(testTemplate).isNull()
  }

  @Test
  fun `getStyleTemplate returns the style template which contains a reference to the service template`() {
    val testTemplate = templateRenderService.getStyleTemplate()
    assertThat(testTemplate).contains("serviceTemplate")
    // todo add full template check?
  }

  @Test
  fun `renderTemplate renders a style template given a service template`() {
    val renderedStyleTemplate = templateRenderService.renderTemplate("test-service", testServiceTemplateData)

    assertThat(renderedStyleTemplate).isNotNull()
    assertThat(renderedStyleTemplate).contains("<style>")
    assertThat(renderedStyleTemplate).contains("</style>")
    assertThat(renderedStyleTemplate).contains("<td>Test Key:</td><td>testValue</td>")
    assertThat(renderedStyleTemplate).contains("<td>Nested Data:</td><td>nestedValue</td>")
    assertThat(renderedStyleTemplate).contains("<td>Array Data:</td><td><ul><li>arrayValue1-1</li><li>arrayValue1-2</li></ul></td>")
    assertThat(renderedStyleTemplate).contains("<td>Test Key:</td><td>testValue2</td>")
    assertThat(renderedStyleTemplate).contains("<td>Nested Data:</td><td>nestedValue2</td>")
    assertThat(renderedStyleTemplate).contains("<td>Array Data:</td><td><ul><li>arrayValue2-1</li><li>arrayValue2-2</li></ul></td>")
  }

  @Test
  fun `renderTemplate renders a template given a keyworker template`() {
    val renderedStyleTemplate = templateRenderService.renderTemplate("keyworker-api", testKeyworkerServiceData)
  ...
  fun `renderTemplate renders a template given an activities template`() {
    val renderedStyleTemplate =
      templateRenderService.renderTemplate("hmpps-activities-management-api", testActivitiesServiceData)
    ...
    assertThat(renderedStyleTemplate).contains("<td class=\"data-column-25\">Status date</td>")
    assertThat(renderedStyleTemplate).contains("<h3>Appointment</h3>")
  }
...
//    assertThat(renderedStyleTemplate).contains("Moorland (HMP & YOI)")
  ...
  fun `renderTemplate renders a template given an adjudications template`() {
    val renderedStyleTemplate =
      templateRenderService.renderTemplate("hmpps-manage-adjudications-api", testAdjudicationsServiceData)

    assertThat(renderedStyleTemplate).isNotNull()
    assertThat(renderedStyleTemplate).contains("<style>")
    assertThat(renderedStyleTemplate).contains("</style>")
    assertThat(renderedStyleTemplate).contains("<td>Prisoner number</td><td>A3863DZ</td>")
    ...
    assertThat(renderedStyleTemplate).contains("<td>Completed</td><td>true</td>")
    assertThat(renderedStyleTemplate).contains("<td>Status</td><td>CHARGE_PROVED</td>")
    assertThat(renderedStyleTemplate).contains("<td>ELECTRICAL_REPAIR</td>")
    assertThat(renderedStyleTemplate).contains("<td>BAGGED_AND_TAGGED</td>")
    assertThat(renderedStyleTemplate).contains("<td>OIC hearing type</td><td>INAD_ADULT</td>")
    assertThat(renderedStyleTemplate).contains("<td>James Warburton</td>")
    assertThat(renderedStyleTemplate).contains("<td>Code</td><td>CHARGE_PROVED</td>")
    assertThat(renderedStyleTemplate).contains("<td class=\"data-column-30\">Privilege type</td>")
    assertThat(renderedStyleTemplate).contains("<td>Linked charge numbers</td><td>[9872-1, 9872-2]</td>")
    assertThat(renderedStyleTemplate).contains("<td>DAYS</td>")
    assertThat(renderedStyleTemplate).contains("<td>Some info</td>")
    assertThat(renderedStyleTemplate).contains("<td>Reason for change</td><td>APPEAL</td>")
  }

  @Test
  fun `renderTemplate renders a template given a home detentions curfew template`() {
    val renderedStyleTemplate = templateRenderService.renderTemplate("hmpps-hdc-api", testHDCServiceData)
  ...
  fun `renderTemplate renders a template given a Use of Force template`() {
    ...
    assertThat(renderedStyleTemplate).contains("<style>")
    assertThat(renderedStyleTemplate).contains("</style>")
    assertThat(renderedStyleTemplate).contains("<td class=\"data-column-25\">Incident date</td>")
  ...
  fun `renderTemplate renders a template given a prepare someone for release template`() {
    val renderedStyleTemplate =
      templateRenderService.renderTemplate("hmpps-resettlement-passport-api", testResettlementPassportServiceData)
  ...
  fun `renderTemplate renders a template given a court case service template`() {
    val renderedStyleTemplate = templateRenderService.renderTemplate("court-case-service", testCourtCaseServiceData)
  ...
  fun `renderTemplate renders a template given a accredited programme service template`() {
    val renderedStyleTemplate =
      templateRenderService.renderTemplate("hmpps-accredited-programmes-api", testAccreditedProgrammesServiceData)
  ...
  fun `renderTemplate renders a template given a Non-associations template`() {
    val renderedStyleTemplate =
      templateRenderService.renderTemplate("hmpps-non-associations-api", testNonAssociationsServiceData)
  ...
  fun `renderTemplate renders a template given a Interventions Service template`() {
    val renderedStyleTemplate =
      templateRenderService.renderTemplate("hmpps-interventions-service", testInterventionsServiceData)
  ...
  fun `renderTemplate renders a template given a Categorisation Service template`() {
    val renderedStyleTemplate =
      templateRenderService.renderTemplate("hmpps-offender-categorisation-api", testCategorisationServiceData)
