...
class GeneratePdfServiceBookASecureMoveTest(
...
) : DescribeSpec(
  {
    fun writeAndThenReadPdf(
      testInput: ArrayList<Map<String, Map<String, Any>>>?,
    ): PdfDocument {
      val testFileName = "dummy-template-book-a-secure-move.pdf"
      val testResponseObject = listOf(DpsService(name = "hmpps-book-secure-move-api", content = testInput))
    ...
    describe("generatePdfService") {
      it("renders for Book a Secure Move API with no data held") {
      ...
      it("renders for Book a Secure Move API with data missing") {
      ...
      it("renders for Book a Secure Move API") {
        val testInput = arrayListOf(
          mapOf(
            "data" to mapOf(
              "id" to "839efad1-fe68-4faf-9d15-b2264a3d3d5b",
