...
class GeneratePdfServiceBookASecureMoveV2Test : BasePdfServiceTest() {
  ...
  fun `PDFService renders Book a Secure Move API with data missing`() {
    val pdfStream = generatorPdfForServices(
      listOf(DpsService(name = "hmpps-book-secure-move-api", content = Stubs.bookAMoveWithDataMissing)),
    )
    ...
    assertPdfContainsExpectedContent(pdfStream) { pdfDocument ->
      ...
      page.assertContainsTextOnce(
        "Moves",
        "Expected Moves section in body but none found",
      )

      page.assertContainsPattern(
        pattern = "Additional information\\s+example Court to Prison prison_remand: Huddersfield Youth Court \nto HMP Isle of Wight",
        errorMessage = "Expected Moves section in body but none found",
      )

      page.assertContainsPattern(
        pattern = "Title\\s+Location\\s+type\\s+Premise\\s+Locality",
        errorMessage = "Location title data not as expected",
      )

      page.assertContainsPattern(
        pattern = "Pentonville\\s+\\(HMP\\)\\s+prison\\s+EitWwGgKEitWwGg\\s+Fazakerley",
        errorMessage = "Location data not as expected",
      )
    }
  }
  ...
  fun `PDFService renders Book a Secure Move API with a full data response from the API`() {
    val pdfStream = generatorPdfForServices(
      listOf(DpsService(name = "hmpps-book-secure-move-api", content = Stubs.bookAMoveFullData)),
    )

    assertPdfContainsExpectedContent(pdfStream) { pdfDocument ->
      val page4 = pdfDocument.getPageText(4)
      page4.assertContainsTextOnce(
        "Name: $subjectName",
        "Subject name incorrect",
      )
      ...
      page5.assertContainsPattern(
        pattern = "Plymouth Crown Court\\s+court\\s+No Data Held\\s+No Data Held",
        errorMessage = "Expected Journey section in body but none found",
      )

      page5.assertContainsPattern(
        pattern = "Vehicle registration\\s+AB12 CDE",
        errorMessage = "expected vehicle reg but not found",
      )
    }
  }
