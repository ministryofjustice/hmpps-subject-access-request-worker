...
class GeneratePdfServiceConsiderRecallTest : BasePdfServiceTest() {

  @Test
  fun `PdfService generates the expected PDF when no data is held`() {
    val pdfStream = generatorPdfForServices(listOf(DpsService(name = "make-recall-decision-api", content = null)))

    assertPdfContainsExpectedContent(pdfStream) { pdfDocument ->
      assertPageOneContainsExpectedContent(
        pdfDocument,
        listOf("make-recall-decision-api"),
      )

      val page = pdfDocument.getPageText(4)
      page.assertContainsTextOnce("Consider a Recall")
      page.assertDoesNotContainsText("CRN")
      page.assertContainsTextOnce("No data held")
    }
  }

  @Test
  fun `PdfService generates the expected PDF for consider a recall data`() {
    val pdfStream = generatorPdfForServices(
      listOf(DpsService(name = "make-recall-decision-api", content = considerRecallData)),
    )
    ...
    assertPdfContainsExpectedContent(pdfStream) { pdfDocument ->
      ...
      var page = pdfDocument.getPageText(4)
      page.assertContainsTextOnce("Consider a Recall")
      page.assertContainsPattern("CRN\\s+Sensitive\\s+PPUD record present\nX098092\\s+false\\s+false")
      page.assertContainsPattern("NO_RECALL\\s+hacking the monitor won't do anything, we need to reboot the digital TLS \ncapacitor!")

      page = pdfDocument.getPageText(5)
      page.assertContainsTextOnce("Custody status")
      page.assertContainsTextOnce("If we transmit the interface, we can get to the PCI pixel through the primary CSS microchip!")

      page = pdfDocument.getPageText(15)
      page.assertContainsTextOnce("Booking momento")
    }
  }

  companion object {
    val considerRecallData = mapOf(
