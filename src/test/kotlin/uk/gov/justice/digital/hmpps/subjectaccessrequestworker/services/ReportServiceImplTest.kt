...
class ReportServiceImplTest {
  ...
  inner class SuccessScenarios {
    ...
    fun `should make expected calls`(): Unit = runBlocking {
      subjectAccessRequest.services.addAll(
        listOf(
          createRequestServiceDetail(unsuspendedServiceConfig, RenderStatus.PENDING),
          createRequestServiceDetail(unsuspendedServiceConfigTwo, RenderStatus.ERRORED),
          createRequestServiceDetail(unsuspendedServiceConfigThree, RenderStatus.COMPLETE),
          createRequestServiceDetail(unsuspendedServiceConfigFour, RenderStatus.PENDING),
          createRequestServiceDetail(unsuspendedServiceConfigFive, RenderStatus.COMPLETE),
        ),
      )
      ...
      thenEventTrackedForServicesSelected("service-4,service-1,service-2")
  ...
  inner class ErrorScenarios {
    ...
    fun `should throw exception when selected service is suspended`(): Unit = runBlocking {
      subjectAccessRequest.services.add(createRequestServiceDetail(suspendedServiceConfig, RenderStatus.PENDING))
      ...
      thenEventTrackedForServicesSelected("service-1")
    }

    @Test
    fun `should throw exception and mark service as errored when render request fails`(): Unit = runBlocking {
      subjectAccessRequest.services.add(createRequestServiceDetail(unsuspendedServiceConfig, RenderStatus.PENDING))
      val exception = SubjectAccessRequestException("test")
      whenever(htmlRendererApiClient.submitRenderRequest(subjectAccessRequest, unsuspendedServiceConfig)).thenThrow(
        exception,
      )
      ...
      verify(subjectAccessRequestService).updateServiceStatusFailed(
        ...
        unsuspendedServiceConfig.serviceName,
      )
      thenEventTrackedForServicesSelected("service-1")
      thenEventTrackedForSubmitRenderRequest(unsuspendedServiceConfig)
    }
