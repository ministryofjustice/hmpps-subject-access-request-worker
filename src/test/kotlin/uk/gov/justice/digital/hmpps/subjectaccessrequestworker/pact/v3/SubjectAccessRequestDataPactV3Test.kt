...
import uk.gov.justice.digital.hmpps.subjectaccessrequestworker.pact.EXPECTED_PRN
import uk.gov.justice.digital.hmpps.subjectaccessrequestworker.pact.SarResponseEntity
...
class SubjectAccessRequestDataPactV3Test : BasePactTest() {
  ...
  companion object {
    ...
    val exclusions = listOf("prn")
    val mutations = mapOf(
      "content[0].creationDateTime" to CONTENT_CREATION_DATE_TIME,
      "content[0].amendments[0].creationDateTime" to AMENDMENT_CREATION_DATE_TIME,
    )
  }
  ...
  fun verifyTemplateTest(mockServer: MockServer) {
    ...
    val jsonRoot = JSONObject(GSON.toJson(sarResponseEntity))
    jsonRoot.keys().forEachRemaining { key -> handleJsonValue(renderedHtml!!, key, jsonRoot.get(key)) }
  }

  fun handleJsonValue(renderedHtml: String, path: String, obj: Any) {
    when (obj) {
      is JSONObject -> {
        obj.keys().forEachRemaining { key ->
          handleJsonValue(renderedHtml, "$path.$key", obj.get(key))
        }
      }

      is JSONArray -> {
        obj.forEachIndexed { i, element ->
          handleJsonValue(renderedHtml, "$path[$i]", element)
        }
      }

      else -> assertHtmlContains(renderedHtml, path, obj)
    }
  }

  fun assertHtmlContains(renderedHtml: String, path: String, value: Any) {
    println("asserting HTML contains: $path=$value")

    if (exclusions.contains(path)) {
      println("$path excluded from html check, skipping")
      return
    }

    assertThat(renderedHtml)
      .withFailMessage("rendered HTML does not contain: $path=$value")
      .contains(mutations.getOrDefault(path, value).toString())
  }
